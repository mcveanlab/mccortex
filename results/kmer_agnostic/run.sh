...
name_list=(gP.l0     gS.l0      gE.l0         gP.lP.raw  gS.lS.raw  gS.lS.clean gE.lE.clean    gP.lE.clean        gE.lS.clean)
glist=(    perf.ctx  stoch.ctx  stocherr.ctx  perf.ctx   stoch.ctx  stoch.ctx   perfedges.ctx  err_isec_perf.ctx  stoch_err_union.ctx)
llist=(    ''        ''         ''            gP.lP.raw  gS.lS.raw  gS.lS.clean gE.lE.clean    gP.lE.clean        gE.lS.clean)
...
for k in $kmers; do
  ...
  [ ! -f k$k/graphs/stoch.raw.ctx    ] && `getctx $k` build -m $MEM -k $k --sample chr22_17M_18M --seq reads/stoch.fa.gz    k$k/graphs/stoch.ctx               >& k$k/graphs/stoch.ctx.log
  [ ! -f k$k/graphs/stoch.ctx        ] && `getctx $k` clean -m $MEM $clean_thresh --covg-before k$k/stoch.raw.covg.csv --tips $[$k*2] --supernodes=$thresh --out k$k/graphs/stoch.ctx k$k/graphs/stoch.raw.ctx >& k$k/graphs/stoch.ctx.log
  ...
  [ ! -f k$k/graphs/stoch_err_union.ctx ] && `getctx $k` join -m $MEM -o k$k/graphs/stoch_err_union.ctx k$k/graphs/stoch.ctx k$k/graphs/stocherr.ctx >& k$k/graphs/stoch_err_union.ctx.log
done
