...
int ctx_join(CmdArgs *args)
{
  ...
  for(i = 0; i < num_binaries; i++)
  {
    ...
    if(!binary_probe(binary_paths[i], &is_binary, &kmer_size2, bin_num_cols+i,
                     &num_kmers)) {
  ...
  else
  {
    ...
    for(i = 0; i < num_binaries; i++)
      binary_parse_colour_array(binary_paths[i], load_colours[i], bin_num_cols[i]);
