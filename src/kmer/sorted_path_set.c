...
// Isaac: remove this code after you commit
/*
// Remove redundant entries such as duplicates and substrings
// {T,TT,TT} -> {TT}
// {A,C,CG,CGC} -> {A,CGC}
void sorted_path_set_slim(SortedPathSet *set)
{
  const size_t num_members = set->members.len, cbytes = set->cbytes;
  if(num_members == 0) return;

  size_t i, j;
  PathEntry *e0, *e1 = &set->members.data[0];
  PathLen plen2;

  for(i = j = 0; i+1 < num_members; i++)
  {
    e0 = e1;
    e1 = &set->members.data[i+1];
    plen2 = e0->plen < e1->plen ? e0->plen : e1->plen;

    // keep substrings if they support colours not in longer version
    if(binary_seqs_cmp(e0->seq, e0->plen, e1->seq, plen2) != 0 ||
       (plen2 < e1->plen &&
        !packedpath_is_colset_subset(e0->seq-cbytes, e1->seq-cbytes, cbytes)))
    {
      set->members.data[j++] = set->members.data[i];
    }
  }
...
void sorted_path_set_merge(SortedPathSet *out_set, SortedPathSet *in_set,
                           bool rmsubstr, uint8_t *store)
{
  const size_t num_in = in_set->members.len, num_out = out_set->members.len;
  const size_t cbytes = in_set->cbytes;
  size_t i = 0, j = 0, k = 0, plen2;
  int cmp;

  ctx_assert2(in_set->cbytes == out_set->cbytes, "%zu vs %zu",
              in_set->cbytes, out_set->cbytes);

  if(num_in == 0 || num_out == 0 || in_set == out_set) return;

  PathEntry *list0 = in_set->members.data;
  PathEntry *list1 = out_set->members.data;

  while(i < num_in && k < num_out)
  {
    plen2 = rmsubstr && list0[i].plen < list1[k].plen ? list0[i].plen : list1[k].plen;
    cmp = binary_seqs_cmp(list0[i].seq, list0[i].plen, list1[k].seq, plen2);

    // keep substrings if they support colours not in longer version
    if(cmp < 0 ||
       (plen2 < list1[k].plen &&
        !packedpath_is_colset_subset(list0[i].seq-cbytes, list1[k].seq-cbytes, cbytes)))
    {
      list0[j++] = list0[i++];
    }
    else if(cmp == 0) {
      // Copy updated colourset if passed store
      if(packedpath_colsets_or(list1[k].seq-cbytes, list0[i].seq-cbytes, cbytes) &&
         store != NULL)
      {
        uint8_t *ptr = packedpath_get_colset(store + list1[k].pindex);
        memcpy(ptr, list1[k].seq-cbytes, cbytes);
      }
      i++;
    }
    else {
  ...
  while(i < num_in) list0[j++] = list0[i++];

  in_set->members.len = j;
}
*/
