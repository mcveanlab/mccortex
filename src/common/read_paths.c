...
{
  uint64_t index = wlk->db_graph->kmer_paths[wlk->node];
                       ...
                       hkey_t node, Orientation orient)
{
  ...
  GraphWalker gw = {.db_graph = graph, .colour = colour,
                    ...
                    .num_paths = 0, .paths_capacity = paths_capacity};

  memcpy(wlk, &gw, sizeof(GraphWalker));
  memcpy(wlk->bkmer, db_graph_bkmer(graph, node), sizeof(BinaryKmer));
...
boolean graph_traverse(GraphWalker *wlk)
{
  ...
  Edges edges = db_graph->edges[wlk->node];

  hkey_t nodes[4];
  BinaryKmer bkmers[4];
  Orientation orients[4];

  int num_next = db_graph_get_next_nodes(wlk->db_graph,
                                         wlk->bkmer, wlk->orient, edges,
                             ...
                             Orientation orients[4])
{
  ...
  // num_next > 0
  if(wlk->num_paths == 0) return false;

  // num_next > 0, num_paths > 0
  uint32_t kmer_size = wlk->db_graph->kmer_size;
  size_t i, j;
  ...
  {
    nuc = db_node_last_nuc(bkmers[i], orients[i], kmer_size);
    if(nuc == greatest_nuc)
    {
      wlk->node = nodes[i];
  ...
  if(i == num_next) die("Something went wrong.");
